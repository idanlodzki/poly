...
def load_betting_config() -> dict:
    conn = _get_conn()
    row = conn.execute("SELECT auto_trade_enabled, threshold, bet_amount FROM betting_config WHERE id = 1").fetchone()
    if row is None:
        return {"auto_trade_enabled": False, "threshold": 10.0, "bet_amount": 10.0}
    return {
...
def save_betting_config(enabled: bool, threshold: float, bet_amount: float = 10.0) -> None:
    conn = _get_conn()
    conn.execute(
        "INSERT INTO betting_config (id, auto_trade_enabled, threshold, bet_amount) VALUES (1, ?, ?, ?) "
        "ON CONFLICT(id) DO UPDATE SET auto_trade_enabled = excluded.auto_trade_enabled, threshold = excluded.threshold, bet_amount = excluded.bet_amount",
